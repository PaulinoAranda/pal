...
namespace pal
{
    ...
    Layer*
    Pal::getLayer ( const char *lyrName)
    {
        try {

            std::cout << "getLayer lyrsMutex lock" << std::endl;
            ...
            lyrsMutex->unlock ();
            std::cout << "getLayer lyrsMutex unlock" << std::endl;
    ...
    void
    Pal::removeLayer ( Layer *layer)
    {
        try{
            std::cout << "removeLayer lyrsMutex lock" << std::endl;
            ...
            lyrsMutex->unlock ();
            std::cout << "removeLayer lyrsMutex unlock" << std::endl;
    ...
    Layer*
    Pal::addLayer ( const char *lyrName,
                    ...
                    bool toLabel)
    {
        ...
        try
        {
            std::cout << "addLayer lyrsMutex lock" << std::endl;
            ...
            lyrsMutex->unlock ();
            std::cout << "addLayer lyrsMutex unlock" << std::endl;
    ...
    typedef struct _featCbackCtx
    {
            ...
            long double bbox_min[2];
            long double bbox_max[2];
            Units unit;
#ifdef _EXPORT_MAP_
            std::ofstream *svgmap;
#endif
                          ...
                          void *ctx)
    {
        ...
        long int dpi = context->layer->pal->getDpi();
#endif
        ...
        if (context->layer->toLabel)
        {
            ...
            if (ft_ptr->label_x > 0.0000001 && ft_ptr->label_y > 0.0000001)
            {
                ...
                if ((ft_ptr->type == GEOS_LINESTRING) || ft_ptr->type == GEOS_POLYGON)
                {

                    long double bbx[4], bby[4];
    ...
    long double
    dist_pointToLabel ( long double xp,
                        long double yp,
                        LabelPosition *lp)
    {
                                 ...
                                 void *ctx)
    {
        ...
        long double scale = (( PruneCtx*) ctx)->scale;
        ...
        long double distlabel = unit_convert (double (lp->feature->distlabel), pal::PIXEL, pal->map_unit, pal->dpi, scale, 1);
        ...
        switch (feat->type)
        {
            ...
            case GEOS_LINESTRING:
                ...
                for ( i = 0; i < 4; i++)
                {
                    ...
                    {
                        ca = cross_product (lp->x[i], lp->y[i], lp->x[(i + 1) % 4], lp->y[(i + 1) % 4], feat->x[j], feat->y[j]);
                        cb = cross_product (lp->x[i], lp->y[i], lp->x[(i + 1) % 4], lp->y[(i + 1) % 4], feat->x[j + 1], feat->y[j + 1]);

                        if ((ca < 0 && cb > 0) || (ca > 0 && cb < 0))
                        {
                            ca = cross_product (feat->x[j], feat->y[j], feat->x[j + 1], feat->y[j + 1], lp->x[i], lp->y[i]);
                            cb = cross_product (feat->x[j], feat->y[j], feat->x[j + 1], feat->y[j + 1], lp->x[(i + 1) % 4], lp->y[(i + 1) % 4]);
                            if ((ca < 0 && cb > 0) || (ca > 0 && cb < 0))
                            {
                                n = 1;
                                i = 4;
    ...
    {
        /*
         RTree<PointSet*, long  double, 2, long  double>::Iterator it;
        ...
        amin[0] = amin[1] = -LDBL_MAX;
        amax[0] = amax[1] = LDBL_MAX;
    ...
    {
            RTree< LabelPosition*, long double, 2, long double> *cdtsIndex;
            long double scale;
            Pal *pal;
    } FilterContext;

    bool
    filteringCallback ( PointSet *pset,
                        void *ctx)
    {

        RTree< LabelPosition*, long double, 2, long double> *cdtsIndex = (( FilterContext*) ctx)->cdtsIndex;
        long double scale = (( FilterContext*) ctx)->scale;
     ...
     */
    Problem*
    Pal::extract ( long int nbLayers,
                   char **layersName,
                   long double *layersFactor,
    ...
    {
        // to store obstacles
        RTree< PointSet*, long double, 2, long double> *obstacles = new RTree< PointSet*, long double, 2, long double> ();
        ...
        {
#ifdef _VERBOSE_
            std::cout << "extract lyrsMutex lock" << std::endl;
#endif
            ...
            for ( i = 0; i < nbLayers; i++)
            {
                ...
                {     // iterate on pal->layers
                    ...
                    if (layer->active && (layer->obstacle || (layer->toLabel && layer->isScaleValid (scale))))
                    {
                        ...
                        if (strcmp (layersName[i], layer->name) == 0)
                        {
                            ...
                            *svgmap << "<g inkscape:label=\"" << layer->name << "\"" << std::endl
                            ...
                            try {
#ifdef _VERBOSE_
                                std::cout<<"extract lock "<<std::endl;
#endif
...
#ifdef _VERBOSE_
                                std::cout<<"extract unlock "<<std::endl;
#endif
...
#ifdef _VERBOSE_
            std::cout << "extract lyrsMutex unlock" << std::endl;
#endif
        ...
        prob->nblp = 0;
        prob->featNbLp = new long int[prob->nbft];
        prob->featStartId = new long int[prob->nbft];
        prob->inactiveCost = new long double[prob->nbft];
        ...
        idlp = 0;
        long int nbOverlaps = 0;
        prob->labelpositions = new LabelPosition*[prob->nblp];
        //prob->feat = new long int[prob->nblp];
        ...
        {     // foreach feature
            ...
            {     // foreach label candidate
                ...
                if (lp->cost >= 1)
                {
                    ...
                    lp->cost -= ( long int) (lp->cost);     // label cost up to 1
                }
    ...
    std::list< Label*>*
    Pal::labeller ( long double scale,
                    long double bbox[4],
                    PalStat **stats,
                    bool displayAll)
    {
...
#endif
        long int i;
        std::list< Label*> *solution = NULL;

        try
        {
#ifdef _VERBOSE_
            std::cout << "labeller lyrsMutex lock" << std::endl;
#endif

            lyrsMutex->lock ();
            long int nbLayers = layers->size ();

            char **layersName = new char*[nbLayers];
            long double *priorities = new long double[nbLayers];
            Layer *layer;
            i = 0;
            for ( std::list< Layer*>::iterator it = layers->begin (); it != layers->end (); it++)
            {
...
#ifdef _VERBOSE_
            std::cout << "labeller lyrsMutex unlock" << std::endl;
#endif


            solution = labeller (nbLayers, layersName, priorities, scale, bbox, stats, displayAll);
#ifdef _VERBOSE_

            std::cout << "labeller solution" << std::endl;
#endif
     ...
     */
    std::list< Label*>*
    Pal::labeller ( long int nbLayers,
                    char **layersName,
                    long double *layersFactor,
                    long double scale,
                    long double bbox[4],
    ...
    void
    Pal::setPointP ( long int point_p)
    {
        if (point_p > 0)
    ...
    void
    Pal::setEjChainDeg ( long int degree)
    {
        this->ejChainDeg = degree;
    }

    void
    Pal::setTenure ( long int tenure)
    {
        this->tenure = tenure;
    }

    void
    Pal::setCandListSize ( long double fact)
    {
        this->candListSize = fact;
    }

    void
    Pal::setDpi ( long int dpi)
    {
        if (dpi > 0)
            this->dpi = dpi;
    }
    ...
    void
    Pal::setSearch ( SearchMethod method)
    {
        switch (method)
        {
            case POPMUSIC_CHAIN:
     ...
     */
    void
    Pal::setMapUnit ( Units map_unit)
    {
        if (map_unit == pal::PIXEL || map_unit == pal::METER || map_unit == pal::FOOT || map_unit == pal::DEGREE)
        {
