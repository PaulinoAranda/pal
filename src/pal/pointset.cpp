...
namespace pal
{
    ...
    PointSet::PointSet ( long int nbPoints,
                         long double *x,
                         long double *y)
    {
        this->nbPoints = nbPoints;
        this->x = new long double[nbPoints];
        this->y = new long double[nbPoints];
        long int i;
    ...
    PointSet::PointSet ( long double x,
                         long double y)
    {
        nbPoints = cHullSize = 1;
        this->x = new long double[1];
        this->y = new long double[1];
        this->x[0] = x;
        this->y[0] = y;
    ...
    PointSet::PointSet ( PointSet &ps)
    {
        ...
        x = new long double[nbPoints];
        y = new long double[nbPoints];

        if (ps.status)
            status = new long int[nbPoints];
        else
            status = NULL;
    ...
    PointSet::~PointSet ()
    {
     ...
     long int
    PointSet::getPath ( long int start,
                        long int stop,
                        long int *path_val)
    {
        long int nbPt = 0;
    ...
    PointSet*
    PointSet::extractPath ( long int path,
                            long int nbPtPath,
                            long int nbBboxPt,
                            long double bbx[4],
                            long double bby[4],
                            ...
                            long int startPt)
    {
        ...
        if (path == -1 || path == 0)
        {
            ...
            newShape->x = new long double[newShape->nbPoints];
            newShape->y = new long double[newShape->nbPoints];
            newShape->status = new long int[newShape->nbPoints];
            ...
            newShape->xmin = LDBL_MAX;
            newShape->xmax = -LDBL_MAX;
            newShape->ymin = LDBL_MAX;
            newShape->ymax = -LDBL_MAX;
    ...
    void
    PointSet::reduceLine ( PointSet *line,
                           ...
                           long double bby[4])
    {
        ...
        long int ba, bb;
        Crossing *crossing;
        long double crossingX, crossingY;
        LinkedList< Crossing*> *crossings = new LinkedList< Crossing*> (ptrCrossingCompare);

        long int nbPoints = line->nbPoints;
        long double *x = line->x;
        long double *y = line->y;
        long int *status = line->status;
        ...
        {
            for ( ba = 0; ba < 4; ba++)
            {     // for each bbox segement
                bb = (ba + 1) % 4;

#ifdef _DEBUG_FULL_
                std::cout << "Test: " << i << "->" << j << " and " << ba << "-->" << bb  << std::endl;
#endif
                if (computeSegIntersection (x[i], y[i], x[j], y[j], bbx[ba], bby[ba], bbx[bb], bby[bb], &crossingX, &crossingY))
                {     // seg[i->j] crossing bbox[ba->bb]
        ...
        if (crossings->size () > 0)
        {
            ...
            while (crossings->size () > 0)
            {
                ...
                {     //  inside
                    ...
                    new_line->x = new long double[new_line->nbPoints];
                    new_line->y = new long double[new_line->nbPoints];
     ...
     */
    void
    PointSet::reducePolygon ( PointSet *shape_toProcess,
                              LinkedList< PointSet*> *shapes_final,
                              long double bbx[4],
                              long double bby[4])
    {
        ...
        {     // foreach feature in toProcess
            ...
            {     // foreach polygon segment
                ...
                {     // for each bbox segement
                    ...
                    if (computeSegIntersection (shape->x[i], shape->y[i], shape->x[j], shape->y[j], bbx[ba], bby[ba], bbx[bb], bby[bb], &crossingX, &crossingY))
                    {     // seg[i->j] crossing bbox[ba->bb]
                        ...
                        if (vabs (cross_product (bbx[ba], bby[ba], bbx[bb], bby[bb], shape->x[i], shape->y[i])) > 0.00001)
                        {
                            ...
                            if (shape->status[i] != ba + 10 && shape->status[j] != ba + 10 && shape->status[i] != 15 && shape->status[j] != 15)
                            {
                                ...
                                if (cross_product (shape->x[crossing->pt], shape->y[crossing->pt], shape->x[(crossing->pt + 1) % shape->nbPoints], shape->y[(crossing->pt + 1) % shape->nbPoints],
                                                   bbx[ba], bby[ba]) > 0)
                                {
            ...
            i = 0;
            long int crossingTabSize = crossings->size ();
            ...
            if (crossingTabSize > 1)
            {
                ...
                if (vabs (a->x - b->x) > 0.00001 || vabs (a->y - b->y) > 0.00001)
                {
                    ...
                    if (j == 4)
                    {
                        if (isSegIntersects (a->x, a->y, bbx[a->nextCorner], bby[a->nextCorner], shape->x[b->pt], shape->y[b->pt], shape->x[(b->pt + 1) % shape->nbPoints],
                                             shape->y[(b->pt + 1) % shape->nbPoints]))
                        {
                            j = 0;
                        }
                    }

                    long int nbBboxPoint = j;
                    long int nbPtPathA = 0;
                    long int nbPtPathB = 0;
                    ...
                    nbPtPathA = shape->getPath ((b->pt + 1) % shape->nbPoints, a->pt, &path_a);

                    nbPtPathB = shape->getPath ((a->pt + 1) % shape->nbPoints, (b->pt + 1) % shape->nbPoints, &path_b);
                    ...
                    {
                        if (path_a == -1)
                        {     // new shape inside => push into shapes_final
                            shapes_final->push_back (newShape);
                        }
                        else
                        {     // to drop or to resplit...
            ...
            else if (nbCrossingIter == 1 || crossingTabSize == 1)
            {     // First shape dont cross the bbox => outside or bbox is inside shape
                ...
                if (check && isPointInPolygon (shape->nbPoints, shape->x, shape->y, (bbx[0] + bbx[2]) / 2, (bby[0] + bby[2]) / 2))
                {
                    // bbox is inside the shape => reduce shape to bbox
                    delete[] shape->status;
                    delete[] shape->x;
                    delete[] shape->y;
                    shape->status = new long int[4];
                    shape->x = new long double[4];
                    shape->y = new long double[4];
                    shape->nbPoints = 4;
                    for ( i = 0; i < 4; i++)
                    {
                        shape->status[i] = 0;
...
//#undef _DEBUG_FULL_
    ...
    PointSet*
    PointSet::extractShape ( long int nbPtSh,
                             long int imin,
                             long int imax,
                             long int fps,
                             long int fpe,
    ...
    void
    PointSet::splitPolygons ( LinkedList< PointSet*> *shapes_toProcess,
                              LinkedList< PointSet*> *shapes_final,
                              long double xrm,
                              long double yrm,
                              char *uid)
    {
        ...
        long int holeS = -1;     // hole start and end points
        long int holeE = -1;

        long int retainedPt = -1;
        long int pt = 0;

        long double labelArea = xrm * yrm;
        ...
        while (shapes_toProcess->size () > 0)
        {
            ...
            for ( ihs = 0; ihs < cHullSize; ihs++)
            {
                ...
                ipn = (ips + 1) % nbp;
                if (ipn != cHull[ihn])
                {     // next point on shape is not the next point on cHull => there is a hole here !
                    bestcp = 0;
                    pt = -1;
                    // lookup for the deepest point in the hole
                    for ( i = ips; i != cHull[ihn]; i = (i + 1) % nbp)
                    {
                        cp = vabs (cross_product (x[cHull[ihs]], y[cHull[ihs]], x[cHull[ihn]], y[cHull[ihn]], x[i], y[i]));
                    ...
                    if (pt != -1)
                    {
                        ...
                        base = dist_euc2d (x[cHull[ihs]], y[cHull[ihs]], x[cHull[ihn]], y[cHull[ihn]]);

                        b = dist_euc2d (x[cHull[ihs]], y[cHull[ihs]], x[pt], y[pt]);

                        c = dist_euc2d (x[cHull[ihn]], y[cHull[ihn]], x[pt], y[pt]);
            ...
            long double cx, cy, dx, dy, ex, ey, fx, fy, seg_length, ptx, pty, fptx = 0, fpty = 0;
            long int ps = -1, pe = -1, fps = -1, fpe = -1;
            if (retainedPt >= 0 && bestArea > labelArea)
            {     // there is a hole so we'll cut the shape in two new shape (only if hole area is bigger than twice labelArea)
#ifdef _DEBUG_FULL_
                ...
                for ( i = (cHull[holeE] + 1) % nbp; i != (cHull[holeS] - 1 + nbp) % nbp; i = j)
                {
...
#endif
                    if (seg_length < EPSILON || vabs ((b = cross_product (ex, ey, fx, fy, x[retainedPt], y[retainedPt]) / (seg_length))) > (seg_length / 2))
                    {     // retainedPt is not fronting i->j
                    ...
                    {     // point fronting i->j => compute pependicular distance  => create a new point
                ...
                long int imin = retainedPt;
                long int imax = (((fps < retainedPt && fpe < retainedPt) || (fps > retainedPt && fpe > retainedPt)) ? min (fps, fpe) : max (fps, fpe));

                long int nbPtSh1, nbPtSh2;     // how many points in new shapes ?
                if (imax > imin)
    ...
    PointSet*
    PointSet::createProblemSpecificPointSet ( long double bbx[4],
                                              long double bby[4],
                                              bool *outside,
                                              bool *inside)
    {
        ...
        shape->status = new long int[nbPoints];
        shape->x = new long double[nbPoints];
        shape->y = new long double[nbPoints];
        ...
        *inside = true;
        ...
        for ( i = 0; i < nbPoints; i++)
        {
            ...
            long double cp0 = cross_product (bbx[0], bby[0], bbx[1], bby[1], shape->x[i], shape->y[i]);
            long double cp1 = cross_product (bbx[1], bby[1], bbx[2], bby[2], shape->x[i], shape->y[i]);
            long double cp2 = cross_product (bbx[2], bby[2], bbx[3], bby[3], shape->x[i], shape->y[i]);
            long double cp3 = cross_product (bbx[3], bby[3], bbx[0], bby[0], shape->x[i], shape->y[i]);
    ...
    CHullBox*
    PointSet::compute_chull_bbox ()
    {
        long int i;
        long int j;

        long double bbox[4];     // xmin, ymin, xmax, ymax

        long double alpha;
        long int alpha_d;

        long double alpha_seg;

        long double dref;
        long double d1, d2;

        long double bb[16];     // {ax, ay, bx, by, cx, cy, dx, dy, ex, ey, fx, fy, gx, gy, hx, hy}}

        long double cp;
        long double best_cp;
        long double distNearestPoint;
        long int nearestPoint;
        ...
        long double best_alpha = -1;
        long double best_bb[16];
        long double best_length = 0;
        long double best_width = 0;
        ...
        for ( alpha_d = 0; alpha_d < 90; alpha_d++)
        {
            ...
            if (best_area - area > EPSILON)
            {
                ...
                best_width = width;
                best_alpha = alpha;
                memcpy (best_bb, bb, sizeof(long double) * 16);
            }
        }
        ...
        for ( i = 0; i < 16; i = i + 4)
        {
            computeLineIntersection (best_bb[i], best_bb[i + 1], best_bb[i + 2], best_bb[i + 3], best_bb[(i + 4) % 16], best_bb[(i + 5) % 16], best_bb[(i + 6) % 16], best_bb[(i + 7) % 16],
...
#if 0
    long double PointSet::getDistInside(double px, long double py){
        ...
        long double width = (xmax - xmin)*2;
        long double height = (ymax - ymin)*2;
    ...
    */
    ...
    /*
     ...
     long double
    PointSet::getDist ( long double px,
                        long double py,
                        long double *rx,
                        long double *ry)
    {
        ...
        long int a, b;
        long int nbP = (type == GEOS_POLYGON ? nbPoints : nbPoints - 1);
        ...
        for ( a = 0; a < nbP; a++)
        {
            ...
            if (computeLineSegIntersection (px, py, px2, py2, x[a], y[a], x[b], y[b], &ix, &iy))
            {
                d = dist_euc2d_sq (px, py, ix, iy);
            }
            else
            {
                long double d1 = dist_euc2d_sq (x[a], y[a], px, py);
                long double d2 = dist_euc2d_sq (x[b], y[b], px, py);
                if (d1 < d2)
                {
                    d = d1;
                    ix = x[a];
    ...
    void
    PointSet::getCentroid ( long double &px,
                            long double &py)
    {
        long double ix, iy;
        long double mesh = min ((xmax - xmin) / 10, (ymax - ymin) / 10);
        bool ptOk = false;
