...
namespace pal
{
    ...
    long int
    Feature::setPositionForPoint ( long double x,
                                   long double y,
                                   long double scale,
                                   LabelPosition ***lPos,
                                   long double delta_width)
    {
        ...
        long int nbp = layer->pal->point_p;
        long int distanceNbp = 3;
        ...
        long double alpha = alphaPAu;
        //        long double beta = 2. * M_PI / (double)nbp; /* angle bw 2 pos */
        long double beta = 2. * M_PI / ( double) nbp; /* angle bw 2 pos */
        ...
        long double distlabel = unit_convert (this->distlabel, pal::PIXEL, layer->pal->map_unit, dpi, scale, delta_width);
        ...
        long double lx, ly; /* label pos */

        /* various alpha */
        long double a90 = M_PI / 2.;
        long double a180 = M_PI;
        long double a270 = a180 + a90;
        long double a360 = 2. * M_PI;
        ...
        *lPos = new LabelPosition*[nbp * distanceNbp];
        ...
        for ( ii = 1; ii <= distanceNbp; ii++)
        {
            ...
            for ( i = 0; i < nbp; i++, (direcionBis ? alpha -= beta : alpha += beta))
            {
                    ...
                    if (i == 1)
                    {
                        if (direcionBis)
                        {
                            if ((direccion && !((( M_PI / 4. - beta / 2. + a360) <= (alpha + beta + a360)) && ((alpha + beta + a360) <= (M_PI / 4. + beta / 2. + a360))))
                                ...
                                ii == 1 ?
                                        cost = 0.0001 :
                                        cost = (0.0002 + (extraCost * 0.0020 * (long double) (icost) / (long double) (nbp - 1))) + ((extraCost * 0.0020 * (long double) (nbp) / (long double) (nbp - 1)) * ( double) (ii - 1));
                            else
                                cost = (0.0002 + (extraCost * 0.0020 * (long double) (icost) / (long double) (nbp - 1))) + ((extraCost * 0.0020 * (long double) (nbp) / (long double) (nbp - 1)) * ( double) (ii - 1));
                        }
                        else
                ...
                (*lPos)[i + ((ii - 1) * nbp)] = new LabelPosition (i + ((ii - 1) * nbp), lx, ly, xrm, yrm, 0, alpha, cost, this);
                //std::cout << ", " << cost ;
                icost += inc;
    ...
    long int
    Feature::setPositionForLine ( long double scale,
                                  LabelPosition ***lPos,
                                  PointSet *mapShape,
                                  long double delta_width)
    {
        ...
        long double *d;     // segments lengths distance bw pt[i] && pt[i+1]
        long double *ad;     // absolute distance bw pt[0] and pt[i] along the line
        long double ll;     // line length
        long double dist;
        long double bx, by, ex, ey;
        long int nbls;
        long double alpha;
        long double cost;
        ...
        d = new long double[nbPoints - 1];
        ad = new long double[nbPoints];
    ...
    long int
    Feature::setPositionForPolygon ( long double scale,
                                     LabelPosition ***lPos,
                                     PointSet *mapShape,
                                     long double delta_width)
    {
        ...
        if (shapes_final->size () > 0)
        {
            ...
            long double dlx, dly;     // delta from label center and bottom-left corner
            long double alpha;     // rotation for the label
            long double px, py;
            long double dx;
            long double dy;
            long int bbid;
            long double beta;
            long double diago = sqrt (xrm * xrm / 4.0 + yrm * yrm / 4);
            long double rx, ry;
    ...
    long int
    Feature::setPosition ( long double scale,
                           LabelPosition ***lPos,
    ...
    )
    {
        ...
        long double delta = bbox_max[0] - bbox_min[0];
