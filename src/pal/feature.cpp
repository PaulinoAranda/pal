...
Feature::Feature(Feat *feat, Layer *layer, int part, int nPart,
...
Feature::~Feature() {
	if (x || y) {
		std::cout << "Warning: coordinates not released: " << layer->name << "/"
				<< (uid ? uid : "uid(null)") << std::endl;
	}
			...
			for (i = 0; i < nbSelfObs; i++) {
				...
				if (selfObs[i]) {
					if (selfObs[i]->x || selfObs[i]->y) {
						std::cout << "Warning: hole coordinates not released"
								<< std::endl;
					}
					delete selfObs[i];
				}
...
int Feature::setPositionForPoint(double x, double y, double scale,
	...
	yrm = unit_convert(label_y, layer->label_unit, layer->pal->map_unit, dpi,
			scale, delta_width);
...
int Feature::setPositionForLine(double scale, LabelPosition ***lPos,
		PointSet *mapShape, double delta_width) {
#ifdef _DEBUG_
...
//shapes_final     = new LinkedList<PointSet*>(ptrPSetCompare);
	...
	LinkedList<LabelPosition*> *positions = new LinkedList<LabelPosition*>(
			ptrLPosCompare);
...
//for (i=0;i<nbp;i++){
	...
	while (l < ll - xrm) {
		...
		if (l < 0)
			birdfly = sqrt(
					(x[nbPoints - 1] - x[0]) * (x[nbPoints - 1] - x[0])
							+ (y[nbPoints - 1] - y[0])
									* (y[nbPoints - 1] - y[0]));
		else
		...
		if (layer->arrangement == P_LINE_AROUND) {
			positions->push_back(
					new LabelPosition(i, bx + cos(beta) * distlabel,
							by + sin(beta) * distlabel, xrm, yrm, alpha,
							alphaPAu, cost, this));     // Line
			positions->push_back(
					new LabelPosition(i, bx - cos(beta) * (distlabel + yrm),
							by - sin(beta) * (distlabel + yrm), xrm, yrm, alpha,
							alphaPAu, cost, this));     // Line
		}
		/*else if (layer->arrangement == P_HORIZ){ // TODO add P_HORIZ
		 ...
		 line->aliveCandidates++;
			...
			positions->push_back(
					new LabelPosition(i, bx - yrm * cos(beta) / 2,
							by - yrm * sin(beta) / 2, xrm, yrm, alpha, alphaPAu,
							cost, this));     // Line
		}
...
int Feature::setPositionForPolygon(double scale, LabelPosition ***lPos,
		PointSet *mapShape, double delta_width) {
	...
	xrm = unit_convert(label_x, layer->label_unit, layer->pal->map_unit,
			layer->pal->dpi, scale, delta_width);

	yrm = unit_convert(label_y, layer->label_unit, layer->pal->map_unit,
			layer->pal->dpi, scale, delta_width);
			...
			for (bbid = 0; bbid < j; bbid++) {
				...
				for (px = px0; px <= box->width; px += dx) {
					for (py = py0; py <= box->length; py += dy) {
						...
						rx = cos(box->alpha) * px
								+ cos(box->alpha - M_PI / 2) * py;
						ry = sin(box->alpha) * px
								+ sin(box->alpha - M_PI / 2) * py;

						rx += box->x[0];
						ry += box->y[0];

						// Only accept candidate that center is in the polygon
						if (isPointInPolygon(mapShape->nbPoints, mapShape->x,
								mapShape->y, rx, ry)) {
							positions->push_back(
									new LabelPosition(0, rx - dlx, ry - dly,
											xrm, yrm, alpha, alphaPAu, 0.0001,
											this)); // Polygon
						}
					}
				}
	...
	if (x && y) {
		...
		for (i = 0; i < nbSelfObs; i++) {
			...
			for (j = 0; j < selfObs[i]->nbPoints; j++) {
				std::cout << selfObs[i]->x[j] << ";" << selfObs[i]->y[j]
						<< std::endl;
			}
		}
	}
...
int Feature::setPosition(double scale, LabelPosition ***lPos,
		double bbox_min[2], double bbox_max[2], PointSet *mapShape,
